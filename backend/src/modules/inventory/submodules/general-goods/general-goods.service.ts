...
export class GeneralGoodsService {
  ...
  async create(dto: CreateGeneralGoodsDto) {
    ...
    const data: any = {
      ...
      quantity:
        (Array.isArray((dto as any).allocations)
          ? ((dto as any).allocations as any[]).reduce((s, a) => s + (a?.quantity ?? 0), 0)
    ...
    const allocations = (dto as any).allocations as any[] | undefined;
    if (Array.isArray(allocations) && allocations.length > 0) {
      ...
      if (rows.length > 0) await this.prisma.inventory.createMany({ data: rows });
...
async findAll(params: {
  ...
  async findOne(id: string) {
    ...
    if (!goods) throw new NotFoundException('General goods item not found');
    return this.mapGeneralGoods(goods);
  }

  async update(id: string, dto: UpdateGeneralGoodsDto) {
