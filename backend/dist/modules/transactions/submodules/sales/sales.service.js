...
let SalesService = class SalesService {
    ...
    async create(dto, userId) {
        ...
        for (const item of dto.items) {
            ...
            if (product.status !== shared_types_1.ProductStatus.IN_STOCK) {
                throw new common_1.BadRequestException(`Product ${item.productId} is not in stock`);
            }
            // Calculate item subtotal
            const itemTotal = (item.goldPrice ?? 0) +
                (item.stonePrice ?? 0) +
        ...
        // Determine status
        let status = shared_types_1.SaleStatus.DRAFT;
        if (dto.status) {
            status = dto.status;
        }
        ...
        if (status === shared_types_1.SaleStatus.COMPLETED) {
    ...
    async findAll(params) {
        const { page, limit, search, status, customerId, branchId, userId, from, to, minAmount, maxAmount, paymentMethod, sortBy = 'saleDate', sortOrder = 'desc', } = params;
        const where = {
            ...(status ? { status } : {}),
    ...
    async update(id, dto) {
        ...
        if (!existing)
            throw new common_1.NotFoundException('Sale not found');
        if (existing.status === shared_types_1.SaleStatus.COMPLETED) {
            throw new common_1.BadRequestException('Cannot update completed sale');
        }
        if (existing.status === shared_types_1.SaleStatus.CANCELLED) {
            throw new common_1.BadRequestException('Cannot update cancelled sale');
        }
    ...
    async recordPayment(id, dto) {
        ...
        if (sale.status === shared_types_1.SaleStatus.CANCELLED) {
            throw new common_1.BadRequestException('Cannot record payment for cancelled sale');
        }
        ...
        if (newPaidAmount >= totalAmount) {
            newStatus = shared_types_1.SaleStatus.COMPLETED;
    ...
    async completeSale(id, notes) {
        ...
        if (!sale)
            throw new common_1.NotFoundException('Sale not found');
        if (sale.status === shared_types_1.SaleStatus.CANCELLED) {
            throw new common_1.BadRequestException('Cannot complete cancelled sale');
        }
        if (sale.status === shared_types_1.SaleStatus.COMPLETED) {
            throw new common_1.BadRequestException('Sale is already completed');
        }
        // Update inventory
        await this.updateInventoryForSale(id, sale.branchId);
        const updated = await this.prisma.sale.update({
    ...
    async cancelSale(id, reason, notes) {
        const sale = await this.prisma.sale.findUnique({ where: { id } });
        if (!sale)
            throw new common_1.NotFoundException('Sale not found');
        if (sale.status === shared_types_1.SaleStatus.COMPLETED) {
            throw new common_1.BadRequestException('Cannot cancel completed sale. Please create a return instead.');
        }
        const updated = await this.prisma.sale.update({
            where: { id },
            data: {
                status: shared_types_1.SaleStatus.CANCELLED,
                notes: `${sale.notes ?? ''}\nCANCELLED: ${reason}\n${notes ?? ''}`.trim(),
            },
        });
        return {
    ...
    async refundSale(id, amount, reason, notes) {
        const sale = await this.prisma.sale.findUnique({ where: { id } });
        if (!sale)
            throw new common_1.NotFoundException('Sale not found');
        if (sale.status !== shared_types_1.SaleStatus.COMPLETED) {
        ...
        if (amount > totalAmount) {
            throw new common_1.BadRequestException('Refund amount exceeds sale total');
        }
        let newStatus;
        if (amount >= totalAmount) {
            newStatus = shared_types_1.SaleStatus.REFUNDED;
        }
        else {
            newStatus = shared_types_1.SaleStatus.PARTIALLY_REFUNDED;
        }
        const updated = await this.prisma.sale.update({
            where: { id },
            data: {
    ...
    async getSummary(from, to, branchId, userId) {
    ...
    async remove(id) {
        ...
        if (!existing)
            throw new common_1.NotFoundException('Sale not found');
        if (existing.status === shared_types_1.SaleStatus.COMPLETED) {
            throw new common_1.BadRequestException('Cannot delete completed sale. Cancel it first.');
        }
        await this.prisma.sale.delete({ where: { id } });
        return { success: true, message: 'Sale deleted' };
    }
    // Helper methods
    async updateInventoryForSale(saleId, branchId) {
        const sale = await this.prisma.sale.findUnique({
            where: { id: saleId },
            include: { items: true },
        });
        ...
        for (const item of sale.items) {
            ...
            await this.prisma.product.update({
                ...
                data: {
                    ...
                    status: shared_types_1.ProductStatus.SOLD,
                },
            });
            // Update inventory
